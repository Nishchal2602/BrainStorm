...
* **Lemma execution engine (optional, additive)** (`platform/src/server/lemma/`): when `LEMMA_ENABLED=true` and configured, **Review PRD** runs a real **[Lemma](https://github.com/lemma-work/lemma-platform) workflow** as the *sequencer* instead of the in-process orchestrator. Lemma is a standalone platform (Docker) whose steps run in its own runtime and **cannot** call our TS agents (verified), so it's used as a **shell**: a `pocket-pm-review` pod workflow with 5 `FORM` nodes; `LemmaReviewRunner` starts the run and, for each `WAITING` node, runs the **same** existing agent in-app + persists (same persistence layer), then advances via `submitForm`. Identical `ReviewRun` lifecycle/`agentStatus`/timeline + a "Executed using Lemma Workflow" card (`__engine`/`__lemmaRunId` stashed in `agentStatus`; no schema change). `lemma-sdk` is isolated behind `LemmaClientPort` (dynamic import; `FakeLemmaClient` for tests). **The orchestrator + agents are unchanged**; the runner **auto-falls-back** to the orchestrator if Lemma is unconfigured/unreachable, so Lemma is never a hard dependency. Setup: `platform/lemma/README.md`. **Do not** port agents into Lemma's runtime (would rewrite them).
